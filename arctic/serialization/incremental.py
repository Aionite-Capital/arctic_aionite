...
class IncrementalPandasToRecArraySerializer(LazyIncrementalSerializer):
    ...
    def _dtype_convert_to_max_len_string(self, input_ndtype, fname):
        # np.string_ and np.unicode_ removed in numpy 2.0, replaced with np.bytes_ and np.str_
        try:
        ...
        if input_ndtype.type not in (string_type, unicode_type):
            return input_ndtype, False
        type_sym = 'S' if input_ndtype.type == string_type else 'U'
